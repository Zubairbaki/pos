                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <h4>
                                    ...
                                    <a href="{{ route('attendance.index') }}" class="btn btn-primary float-sm-right"> <i
                                            class="fas fa-list"></i>Edit Attendance List</a>
                                </h4>
                            </ol>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form action="{{ route('attendance.store') }}" method="post" id="myForm">
                                @csrf
                                <div class="form-group col-md-4">
                                    <label for="date" class="control-label">Attendance Date</label>
                                    <input type="date" name="date" id="date"
                                        value="{{ $attendanceRecord[0]['date'] }}"
                                ...
                                <table class="table sm table-bordered table-striped dt-responsive" style="width: 100%">
                                    ...
                                    <tbody>
                                        @foreach ($attendanceRecord as $key => $item)
                                                ...
                                                <td colspan="3">
                                                    <div class="switch-toggle switch-3 switch-candy">
                                                        <input class="present" id="present{{ $key }}"
                                                            name="attand_status{{ $key }}" value="present"
                                                            type="radio"
                                                            {{ $item->attand_status == 'present' ? 'checked' : '' }}>
                                                        <label for="present{{ $key }}">Present</label>


                                                        <input class="leave" id="leave{{ $key }}"
                                                            name="attand_status{{ $key }}" value="Leave"
                                                            type="radio"  {{ $item->attand_status == 'Leave' ? 'checked' : '' }}>
                                                        <label for="leave{{ $key }}">Leave</label>


                                                        <input class="absent" id="absent{{ $key }}"
                                                            name="attand_status{{ $key }}" value="Absent"
                                                            type="radio"   {{ $item->attand_status == 'Absent' ? 'checked' : '' }}>
